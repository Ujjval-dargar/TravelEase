...
// Modal Functions
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.getElementById("add-itinerary-form").addEventListener("submit", async function(e) {
                ...
                if (data.success) {
                    showModal('Success', 'Itinerary added successfully!', 'success');
                    setTimeout(() => {
                        location.reload();
                    }, 1500);
                } else {
                    showModal('Error', 'Failed to add itinerary: ' + (data.error || 'Unknown error'), 'error');
                }
            });
        
            // Modify Itinerary Form Handler
            document.getElementById("modify-itinerary-form").addEventListener("submit", async function(e) {
                e.preventDefault();
                const formData = new FormData(this);
                const jsonData = Object.fromEntries(formData.entries());
            
                const res = await fetch('/api/modify_itinerary', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify(jsonData)
                });
                ...
                const data = await res.json();
                if (data.success) {
                    showModal('Success', 'Itinerary updated successfully!', 'success');
                    setTimeout(() => {
                        location.reload();
                    }, 1500);
                } else {
        ...
        async function deleteItinerary(itinerary_id) {
            showConfirmModal("Are you sure you want to delete this itinerary?", async function() {
                const res = await fetch('/api/delete_itinerary', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ itinerary_id: itinerary_id })
                });
            
                const data = await res.json();
                if (data.success) {
                    showModal('Success', 'Itinerary deleted successfully.', 'success');
                    setTimeout(() => {
                        location.reload();
                    }, 1500);
                } else {
